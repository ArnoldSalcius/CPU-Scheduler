...
void processQ::Enqueue(process *temp) {
	...
	if (temp == NULL) { // if function returns empty temp, no processes - CPU is Idling
		...
		while (temp->processArray[temp->i] > 0) { //while the burst is more than 0, subtract 1 from current burst
			...
			temp->processArray[temp->i]--;
			time++;
			increment();

		}
		if (temp->processArray[temp->i] == 0) { // when burst reaches 0, i is incremented(moving to the next element)
			temp->i++;
...
struct process* processQ::nextProcess()
{
	...
	while (temp != tail->next) {
		if (min > temp->arrivalTime && temp->state == 1) {//select waiting object with lowest ArrivalTime, set temp2 to that object
			min = temp->arrivalTime;					
			temp2 = temp;
		}
		temp = temp->next;
	}
	return temp2; //return waiting object with lowest arrivalTime or return NULL if empty
}
...
int main() {
	...
	var.Enqueue(var.head); // Enqueue First process
	while (var.isCompleted() == false) {// While isCompleted function returns false, keep going
		var.Enqueue(var.nextProcess());
	}
	var.displayTotals();//Display totals at the end
	system("pause"); //So I dont have to write complicated instructions on how to make this gradeable
